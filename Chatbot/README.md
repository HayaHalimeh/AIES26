...
A conversational AI chatbot that recommends healthy recipes through a guided two-phase interaction. Participants chat with the assistant, receive recipe suggestions, and complete in-app surveys.

**Stack:** React + Vite · Node.js + Express · OpenAI API · ElevenLabs TTS · SQLite · Python recipe recommender
...
└── r3f-virtual-ai-frontend/
    ├── src/
    │   ├── components/                  # UI, SurveyOverlay, WelcomeOverlay
...
Create `Chatbot/.env` on the server:
...
### Root — `Chatbot/.env` (server deployment only)
