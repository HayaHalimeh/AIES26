...
export const ChatProvider = ({ children }) => {
      ...
      if (Array.isArray(res_json.messages) && res_json.messages.length) {
        ...
        const incoming = res_json.messages.map((m) => {
          ...
          if (m?.role === "assistant" && typeof m.text === "string") {
            const { cleanedText, linksByLine } = extractLinksByLineAndClean(m.text);
            return { ...m, text: cleanedText, linksByLine, recipes: m.recipes || null, showSurvey: m.showSurvey || false, surveyPhase: m.surveyPhase || null };
          }
          return m;
        });
    ...
    try {
      ...
      if (Array.isArray(data.messages) && data.messages.length) {
        const incoming = data.messages.map((m) => {
          if (m?.role === "assistant" && typeof m.text === "string") {
            const { cleanedText, linksByLine } = extractLinksByLineAndClean(m.text);
            return { ...m, text: cleanedText, linksByLine, recipes: m.recipes || null, showSurvey: false, surveyPhase: null };
          }
          return m;
        });
  ...
  useEffect(() => {
    if (!message) {
      ...
      setShowText(true);
      // Auto-advance queue after brief delay (no audio playback needed)
